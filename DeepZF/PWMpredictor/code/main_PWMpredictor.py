...
def user_input():
    ...
    parser.add_argument(
        "-in", "--input_file", help="input zinc finger file", type=str, required=True
    )
    parser.add_argument(
        "-out",
        "--output_file",
        help="output PWN predictions file",
        type=str,
        required=True,
    )
    parser.add_argument(
        "-m", "--model_file", help="trained model file", type=str, required=True
    )
...
def main(args):
    ...
    c_rc_df = pd.read_csv(args["input_file"])
