...
def predict_by_len(
    ...
    for len_matching_dataset, seq_len, batch_size in split_dataset_by_len(
        ...
        X, y_true, sample_weights = encode_dataset(
...
def user_input():
    ...
    parser.add_argument(
        "-out", "--output_file", help="results", type=str, required=True
    )
    parser.add_argument(
        "-m", "--model_file", help="trained model", type=str, required=True
    )
